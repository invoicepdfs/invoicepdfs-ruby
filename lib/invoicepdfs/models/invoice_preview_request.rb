  ...
  class InvoicePreviewRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `InvoicePDFs::InvoicePreviewRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `InvoicePDFs::InvoicePreviewRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      page_size_validator = EnumAttributeValidator.new('String', ["LETTER", "A4"])
    ...
    def page_size=(page_size)
      validator = EnumAttributeValidator.new('String', ["LETTER", "A4"])
    ...
    def hash
      [invoice_number, document_type, issue_date, due_date, currency, locale, business_profile_id, customer_id, ship_to, line_items, discounts, shipping, notes, terms, custom_fields, payment, branding, template_id, page_size].hash
    end
