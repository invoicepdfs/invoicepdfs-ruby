  ...
  class DocumentPatchRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `InvoicePDFs::DocumentPatchRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `InvoicePDFs::DocumentPatchRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [number, document_type, issue_date, due_date, currency, locale, business_profile_id, customer_id, source_document_id, reason, ship_to, buyer_reference, tax_scheme, delivery_date, line_items, discounts, shipping, notes, terms, custom_fields, payment, branding].hash
    end
